...
class MaintenanceViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun addMaintenanceLog(
        ...
        nextServiceKm: Double?, warrantyExpiryDate: Long?, warrantyNotes: String, notes: String
    ) {
        viewModelScope.launch {
            repository.insertMaintenanceLog(
                MaintenanceLog(
                    ...
                    garage = garage, nextServiceKm = nextServiceKm,
                    warrantyExpiryDate = warrantyExpiryDate, warrantyNotes = warrantyNotes, notes = notes
                )
            )
            nextServiceKm?.let { km ->
                repository.insertReminder(
                    com.cartracker.data.db.entities.Reminder(
                        carId = carId, title = "Next $serviceType",
                        type = com.cartracker.data.db.entities.ReminderType.MILEAGE,
                        targetMileage = km, notes = "Auto-created from service log"
                    )
                )
            }
        }
